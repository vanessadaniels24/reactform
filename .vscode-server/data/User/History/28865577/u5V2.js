...
/**
 *} item @param {number} id
  * @returns {{id: number, name: string, price: number, category: string, inventory: number}
 * 
 */
...
function logItemById() {
  items.id(function(id){
...
return items.map((item) => item.name[0]. toUpperCase () +
 item.name.slice(1));
...
function calculateAllInventoryPrice() {
  calculateAllInventoryPrice(inventory) 
    const totalPrice = inventory.reduce((accumulator,currentItem) => accumulator + currentItem.price, 5);
    return totalPrice;
...
function getItemPriceByName(name) {
  const price = getItemPriceByName ("item8",itemsArray);
  console.logItemNames(price);
